...
export const getVehiclesByUserId = () => async dispatch => {
  const response = await axios.get("http://localhost:5000/api/vehicle/personal-listings");
...
export const deleteVehicle = vehicle_id => async dispatch => {
  await axios.delete(`http://localhost:5000/api/vehicle/${vehicle_id}`);
...
export const addVehicle = formData => async (dispatch, getState) => {
  ...
  try {
    const response = await axios.post("http://localhost:5000/api/vehicle/", {...formData, ...user}, config);
...
export const updateVehicle = (formData, vehicle_id) => async (dispatch, getState) => {
  ...
  try {
    const response = await axios.patch(`http://localhost:5000/api/vehicle/${formData.vehicle_id}`, {...formData, ...user}, config);
    dispatch({ type: ADD_VEHICLE, payload: response.data });
...
export const getVehicleById = vehicle_id => async dispatch => {
  const response = await axios.get(`http://localhost:5000/api/vehicle/${vehicle_id}`);
...
export const getVehiclesByFavorited = () => async dispatch => {
  const response = await axios.get('http://localhost:5000/api/vehicle/favorites/my-favorites');
  dispatch({ type: GET_VEHICLES, payload: response.data });
}

export const getFavoriteByUserAndVehicle = (vehicle_id) => async dispatch => {
  vehicle_id = parseInt(vehicle_id);
  const response = await axios.get(`http://localhost:5000/api/vehicle/favorite/${vehicle_id}`);
...
export const toggleFavorite = vehicle_id => async dispatch => {
  const response = await axios.patch(`http://localhost:5000/api/vehicle/favorite/${vehicle_id}`);
  dispatch({ type: UPDATE_FAVORITE, payload: {favorite: response.data }});
