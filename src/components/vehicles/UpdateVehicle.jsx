...
import { updateVehicle, getVehicleById } from "../../actions/vehicles";
...
const UpdateVehicle = ({ 
    ...
    useEffect(() => {
        ...
        setFormData({
            title: loading || !vehicle.title ? "" : vehicle.title,
            location: loading || !vehicle.location ? "" : vehicle.location,
            description: loading || !vehicle.description ? "" : vehicle.description,
            perks: loading || !vehicle.perks ? "" : vehicle.perks,
            minBooking: loading || !vehicle.minbooking ? "" : vehicle.minbooking,
            maxBooking: loading || !vehicle.maxbooking ? "" : vehicle.maxbooking,
            price: loading || !vehicle.price ? "" : vehicle.price
        });
    }, [getVehicleById]);
    ...
    const onSubmit = e => {
        e.preventDefault();
        updateVehicle(formData, vehicle_id);
        navigate('/vehicles/my-listings')
    };

    return (
        <Fragment>
            <h1 className="font-bold text-center text-red-500 text-4xl md:text-2xl mt-8 mb-6">Update your listing</h1>
            <form
